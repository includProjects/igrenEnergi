...
class WGL_Posts_Widget extends WP_Widget {
    ...
    function __construct() {
        /* Widget settings. */
        $widget_ops = ['classname' => 'widget_wgl_posts', 'description' => esc_html__('Display recent posts by categories (or related)' , 'irecco-core') ];

        /* Create the widget. */
        parent::__construct( 'wgl-posts', esc_html__('WGL Posts', 'irecco-core'), $widget_ops );
    }
    ...
    function widget( $args, $instance) {
        ...
        echo iRecco_Theme_Helper::render_html($before_widget);

        /* Display the widget title if one was input (before and after defined by themes). */
        if ($title) {
            echo iRecco_Theme_Helper::render_html($before_title);
            echo esc_attr($instance['title']);
            echo iRecco_Theme_Helper::render_html($after_title);
        }
            ...
            <?php while($recent_posts->have_posts()): $recent_posts->the_post(); ?>
                <?php
                    ...
                    if ($show_content == 'true') {
                        ...
                        $text = iRecco_Theme_Helper::modifier_character($without_tags, 50, "...");
                    } else {
                        $text = '';
                    }
                ?>
                ...
                <li class="clearfix<?php echo ((! empty($img_render)) ? ' with_image' : '') ?>">
                    <div class="recent-posts-content">
                        <?php
                            if (! empty($img_render)) {
                                echo '<div class="recent-posts-image_wrapper"><a href="' . esc_url(get_permalink()) . '"><img src="' . esc_url(aq_resize($img_render[0], "168", "168", true, true, true)) . '" alt="' . the_title_attribute([ 'before' => '', 'after' =>  '', 'echo' => false]) . '"></a></div>';
                            }
                        ?>
                        <div class="recent-posts-content_wrapper">
                            <div class="post_title">
                                <a href='<?php esc_url(the_permalink()); ?>' title='<?php esc_attr_e('Permalink to ','irecco-core'); the_title_attribute(); ?>'><?php echo esc_html(the_title($before = '', $after = '', $echo = false)); ?></a>                    
                            </div><!-- post-title -->
    ...
    <?php else : esc_html_e('No posts were found for display','irecco-core');
    endif; ?>

        <?php
        /* After widget (defined by themes). */
        echo iRecco_Theme_Helper::render_html($after_widget);
    ...
    function form($instance)
    {
        ...
        $defaults = [
            'title' => esc_html__('Recent Posts' , 'irecco-core'),
            'num_posts' => 3,
            'categories' => 'all',
            'show_related'  => 'off',
            'show_image' => 'on', 
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:' , 'irecco-core') ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('num_posts')); ?>"><?php esc_html_e('Number of posts:' , 'irecco-core'); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_related')); ?>"><?php esc_html_e('Show related category posts' , 'irecco-core'); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>"><?php esc_html_e('Show thumbnail image' , 'irecco-core'); ?></label>
        </p>

        <p style="margin-top: 20px;">
            <label style="font-weight: bold;"><?php esc_html_e('Post meta info' , 'irecco-core'); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>"><?php esc_html_e('Show date' , 'irecco-core'); ?></label>
        </p>        
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_content')); ?>"><?php esc_html_e('Show content' , 'irecco-core'); ?></label>
        </p>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e('Filter by Category:' , 'irecco-core'); ?></label> 
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                <option value='all' <?php if ('all' == $instance['categories'] ) echo 'selected="selected"'; ?>><?php esc_html_e('All categories' , 'irecco-core');?></option>
