...
if (!class_exists('iRecco_footer_area')) {
    /**
     * Footer area
     *
     *
     * @class		iRecco_footer_area
     * @version		1.0
     * @category	Class
     * @author		WebGeniusLab
     */
    ...
    class iRecco_footer_area
    {
        ...
        function __construct()
        {
            ...
            $copyright_switch = iRecco_Theme_Helper::get_option('copyright_switch');

            $footer_bg_color = iRecco_Theme_Helper::get_option('footer_bg_color');
            $style = '';
            
            // Upper-level variables
            $this->id = get_queried_object_id();
            $this->content_type = iRecco_Theme_Helper::options_compare('footer_content_type', 'mb_footer_switch', 'on');
            $this->footer_full_width = iRecco_Theme_Helper::get_option('footer_full_width');
            ...
            if ($this->content_type == 'widgets' && ($footer_switch || $copyright_switch)) {
                ...
                $style .= iRecco_Theme_Helper::bg_render('footer', 'mb_footer_switch', 'on');
                $style = $style ? ' style="' . esc_attr($style) . '"' : '';
            }


            // Hide on 404 page
            $page_not_found = iRecco_Theme_Helper::get_option('404_show_footer');
        ...
        {
            $wave_switch = iRecco_Theme_Helper::options_compare('footer_add_wave', 'mb_footer_switch', 'on');
            if ($wave_switch) {

                $wave_height = iRecco_Theme_Helper::get_option('footer_wave_height');
                ...
                echo "<div class='irecco_wave_footer' style='height:", (int) $wave_height, "px;'>",
        ...
        private function get_footer_vars($optn_1 = null)
        {
            ...
            // Get options
            $footer_spacing = iRecco_Theme_Helper::options_compare('footer_spacing', 'mb_footer_switch', 'on');
            $footer_border = iRecco_Theme_Helper::options_compare('footer_add_border', 'mb_footer_switch', 'on');
            $footer_border_color = iRecco_Theme_Helper::options_compare('footer_border_color', 'mb_footer_switch', 'on');

            if ($optn_1 == 'widgets') {
                $footer_options['widget_columns'] = iRecco_Theme_Helper::get_option('widget_columns');
                $footer_options['widget_columns_2'] = iRecco_Theme_Helper::get_option('widget_columns_2');
                $footer_options['widget_columns_3'] = iRecco_Theme_Helper::get_option('widget_columns_3');
                $footer_align = iRecco_Theme_Helper::get_option('footer_align');
        ...
        private function main_footer_get_page()
        {
            ...
            $footer_page_select = iRecco_Theme_Helper::options_compare('footer_page_select', 'mb_footer_switch', 'on');
        ...
        {
            // Get options
            $copyright_spacing = iRecco_Theme_Helper::options_compare('copyright_spacing', 'mb_copyright_switch', 'on');
        ...
        private function copyright_style()
        {
            ...
            if ($this->content_type == 'widgets') {
                $bg_color = iRecco_Theme_Helper::options_compare('copyright_bg_color', 'mb_copyright_switch', 'on');
        ...
        {
            $editor = iRecco_Theme_Helper::get_option('copyright_editor');

            if ($this->mb_copyright_switch == 'on') {
                $editor = rwmb_meta('mb_copyright_editor');
            }
            ?>
            <div class='copyright' <?php echo iRecco_Theme_Helper::render_html($this->copyright_style()); ?>>
                <?php if (!$this->footer_full_width) echo "<div class='wgl-container'>"; ?>
                <div class='row' <?php echo iRecco_Theme_Helper::render_html($this->copyright_spacing()); ?>>
