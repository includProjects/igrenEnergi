	...
	class Wgl_Theme_Verify{
		...
		*/
			...
			<div class="notice notice-error is-dismissible">
				<p>
					<?php echo wp_get_theme()->get('Name');
						echo sprintf( esc_html__( ' Theme is not activated! Please activate your copy of the %s Theme', 'irecco'), wp_get_theme()->get('Name') );
						?>
				</p>
				<p>
					<strong style="color:red"><?php esc_html_e( 'Please activate the theme!', 'irecco' ); ?></strong> -
					<a href="<?php echo esc_url(( $page_purchase )); ?>">
						<?php esc_html_e( 'Activate Now','irecco' ); ?> 
					</a> 
				</p>
			</div>
		...
		public function notices_support_until(){
			...
			if( !iRecco_Theme_Helper::wgl_theme_activated() || (bool) ( $wgl_transient = get_transient( 'hide_notice' ) ) ){
				return;
			}
			...
			<div class="notice notice-warning is-dismissible">
				<p class="support-until_text">
					<?php
						...
						if($now > $until_date){
							echo sprintf( wp_kses( __( '<strong>Your support period expired. <a href="%s">You can prolong support</a> services as you need.</strong>', 'irecco' ), $allowed_html), esc_url($item_url));    
						}elseif($now < $until_date && $interval->days < 30){
							echo sprintf( wp_kses( __( '<strong>Your support period will be expired after %d days. <a href="%s">You can prolong support</a> services as you need.</strong>', 'irecco' ), $allowed_html), $interval->days, esc_url($item_url));  
						}
				...
				<p>
					<a href="<?php echo esc_url( wp_nonce_url( add_query_arg('hide_notice', '1' ,admin_url( 'admin-post.php' )), 'hide_notices_nonce', '_notice_nonce' ) ); ?>" class="remind_me_later dismiss_notices">
						<?php esc_html_e( 'Remind Me Later','irecco' ); ?> 
					</a> 
				</p>
			</div>
		...
		public function purchase_activation(){
			...
			if ( ! isset( $_POST['email'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-activation' ) ) {
				$output['error'] = 1; 
				$output['message'] = esc_html__( 'Please enter a valid field', 'irecco' );
				...
				if( ! is_email( $email ) ){
					$output['error'] = 1; 
					$output['message'] = esc_html__( 'Please enter a valid email address.', 'irecco' );
					echo json_encode( $output );     
					wp_die();
				}
				...
				if(empty($purchase)){
					$output['error'] = 1; 
					$output['message'] = esc_html__( 'Purchase code is empty ', 'irecco' );
					echo json_encode( $output );     
					wp_die();                    
				}
				...
				if( $return !== false ){ 
					...
					if(isset($result['success']) && !empty($result['success'])){
						...
						$output['message'] =  esc_html__( 'Thank you, your license has been validated', 'irecco' ); 
		...
		public function dismissed_notice(){
			if ( ! wp_verify_nonce( $_POST['nonce'], '_notice_nonce' ) ) {
				...
				wp_send_json_success( esc_html__( 'Success', 'irecco' ) );
			}
			wp_die();            
		}
