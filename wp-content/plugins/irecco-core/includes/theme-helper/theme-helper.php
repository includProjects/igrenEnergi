...
class WglThemeHelper
{
	...
	public function render_post_share($show_share)
	{
		...
		if ($show_share) : ?>
			...
			<div class="share_social-wpapper">
				...
				<?php
				if ($img_url) {
					echo '<a class="share_link share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url) .'"><span class="fa fa-pinterest-p"></span></a>';
				}
				?>
		...
		<div class="share_post-container">
			<div class="share_social-wpapper">
				<ul>
					...
					if ($img_url) {
						echo '<li>',
							'<a class="share_post share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url) .'"><span class="fa fa-pinterest"></span></a>',
						'</li>';
					}
					?>
					<li>
	...
	{
		$description = esc_html__( 'Socials', 'irecco-core' );

		$facebook  = iRecco_Theme_Helper::options_compare('soc_icon_facebook', 'mb_customize_soc_shares', 'on');
		$twitter   =  iRecco_Theme_Helper::options_compare('soc_icon_twitter', 'mb_customize_soc_shares', 'on');
		$linkedin  = iRecco_Theme_Helper::options_compare('soc_icon_linkedin', 'mb_customize_soc_shares', 'on');
		$pinterest = iRecco_Theme_Helper::options_compare('soc_icon_pinterest', 'mb_customize_soc_shares', 'on');
		$tumblr    = iRecco_Theme_Helper::options_compare('soc_icon_tumblr', 'mb_customize_soc_shares', 'on');

		$fixed  = iRecco_Theme_Helper::options_compare('soc_icon_position', 'mb_customize_soc_shares', 'on');
		$offset = iRecco_Theme_Helper::get_option('soc_icon_offset');
		...
		$share = iRecco_Theme_Helper::options_compare('soc_icon_style', 'mb_customize_soc_shares', 'on');
			...
			<div class="share_social-wpapper">
				...
				if ($share == 'hovered' ) : ?>
					<div class="share_social-desc">
						<div class="share_social-title">
							<?php
							echo apply_filters('irecco_desc_socials', $description);
							?>
						</div>
				...
				<ul>
					...
					if (!empty($share_pinterest)) :
						...
						if ($img_url) {
							echo '<li>',
								'<a class="share_page share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url) .'"><span class="fa fa-pinterest"></span></a>',
							'</li>';
						}
					endif;
					...
					$custom_share = iRecco_Theme_Helper::get_option('add_custom_share');
					if (isset($custom_share) && ! empty($custom_share)) {
						for ( $i = 1; $i <= 10; $i++) {
							${'custom_share_'.$i} = iRecco_Theme_Helper::get_option('select_custom_share_text-'.$i);
							${'custom_share_icons'.$i} = iRecco_Theme_Helper::get_option('select_custom_share_icons-'.$i);
