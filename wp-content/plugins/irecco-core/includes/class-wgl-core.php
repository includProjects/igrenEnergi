...
class iRecco_Core
{
     ...
     * @since    1.0.0
     * @access   protected
     * @var      iRecco_Core_Loader    $loader    Maintains and registers all hooks for the plugin.
     */
    protected $loader;
    ...
    public function wgl_ajax_init()
    {
        ...
        switch ($post_type) {
            ...
            case 'product':
                ...
                if (1 == $total) {
                    $out .= esc_html__('Showing the single result', 'irecco-core');
                } elseif ($total <= $per_page || -1 == $per_page) {
                    $out .= sprintf(esc_html__('Showing all %d results', 'irecco-core'), $total);
                } else {
                    $out .= sprintf(_x('Showing <strong>%1$d&ndash;%2$d</strong> of %3$d results', '%1$d = first, %2$d = last, %3$d = total', 'irecco-core'), $first, $last, $total);
                }
     ...
     * @access   public
     */
    public function wgl_cache_query($args = [])
    {
        $cache_key = http_build_query($args);
        $custom_query = wp_cache_get($cache_key, 'irecco_theme');
        if (false === $custom_query) {
            $custom_query = new WP_Query($args);
            if (!is_wp_error($custom_query) && $custom_query->have_posts()) {
                wp_cache_set($cache_key, $custom_query, 'irecco_theme');
            }
        }
     ...
     *
     * - iRecco_Core_Loader. Orchestrates the hooks of the plugin.
     * - iRecco_Core_i18n. Defines internationalization functionality.
     * - iRecco_Core_Admin. Defines all hooks for the admin area.
     * - iRecco_Core_Public. Defines all hooks for the public side of the site.
     *
    ...
    private function load_dependencies()
    {
        ...
        $this->loader = new iRecco_Core_Loader();
    }

    /**
     * Define the locale for this plugin for internationalization.
     *
     * Uses the iRecco_Core_i18n class in order to set the domain and to register the hook
     * with WordPress.
     *
     * @since    1.0.0
     * @access   private
    ...
    {
        $plugin_i18n = new iRecco_Core_i18n();
    ...
    {
        $plugin_admin = new iRecco_Core_Admin($this->get_plugin_name(), $this->get_version());
    ...
    {
        $plugin_public = new iRecco_Core_Public($this->get_plugin_name(), $this->get_version());
     ...
     *
     * @since     1.0.0
     * @return    iRecco_Core_Loader    Orchestrates the hooks of the plugin.
     */
