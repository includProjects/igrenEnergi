...
class Wgl_Clients extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__('WGL Clients', 'irecco-core');
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section('wgl_clients_section',
            array(
                'label'         => esc_html__('Clients Settings', 'irecco-core'),
            )
        );
        ...
        $this->add_control('item_grid',
            array(
                ...
                'options'           => [
                    ...
                    '6'          => esc_html__('Six Columns', 'irecco-core'),
                ],
                'default'           => '1',
            )
        ); 
        ...
        $repeater->add_control(
            'hover_thumbnail',
            array(
                'label'       => esc_html__('Hover Thumbnail', 'irecco-core'),
                'type'        => Controls_Manager::MEDIA,
                'label_block' => true,
                'default' => [
                    'url' => '',
                ],
                'description' => esc_html__('Need for \'Exchange Images\' and \'Shadow\' animations only.', 'irecco' ),
            )
        );
        ...
        $this->add_control('item_anim',
            array(
                'label'             => esc_html__('Thumbnail Animation', 'irecco-core'),
                'type'              => Controls_Manager::SELECT,
                'options'           => [
                    'none'          => esc_html__('None', 'irecco-core'),
                    'grayscale'          => esc_html__('Grayscale', 'irecco-core'),
                    'opacity'          => esc_html__('Opacity', 'irecco-core'),
                    'zoom'          => esc_html__('Zoom', 'irecco-core'),
                    'contrast'          => esc_html__('Contrast', 'irecco-core'),
                    'blur'          => esc_html__('Blur', 'irecco-core'),
                    'invert'          => esc_html__('Invert', 'irecco-core'),
                    'ex_images'          => esc_html__('Exchange Images', 'irecco-core'),
                    'ex_images_ver'          => esc_html__('Exchange Images Vertical', 'irecco-core'),
                    'ex_images_bg'          => esc_html__('Exchange Images with Background', 'irecco-core'),
                ],
        ...
        $this->add_control(
            'height',
            array(
                'label' => esc_html__('Custom Items Height', 'irecco-core'),
        ...
        $this->add_control(
            'item_align_v',
            array(
                'label' => esc_html__('Vertical Alignment', 'irecco-core'),
                'type' => Controls_Manager::CHOOSE,
                'options' => [
                    'flex-start'      => [
        ...
        $this->start_controls_section(
			'section_style',
			[
				'label' => esc_html__('Carousel', 'irecco-core'),
        ...
        $this->add_control(
			'image_border_radius',
			[
				'label' => esc_html__('Image Border Radius', 'irecco-core'),
        ...
        foreach ($settings[ 'list' ] as $index => $item) {
            ...
            ?><div class="clients_image"><?php 
                if ( !empty($item['client_link']['url']) ) : ?><a <?php echo $this->get_render_attribute_string( $client_link ); ?>><?php 
                    ...
                    if ($url_hover && ($item_anim == 'ex_images' || $item_anim == 'ex_images_bg' || $item_anim == 'ex_images_ver')) : ?><img <?php echo $this->get_render_attribute_string( $client_hover_image ); ?> /><?php endif;
