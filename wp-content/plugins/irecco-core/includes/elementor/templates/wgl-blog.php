...
class WglBlog
{
	...
	public function cache_query($args = [])
	{
		...
		$custom_query = wp_cache_get( $k, 'irecco_theme' );
		if (false === $custom_query) {
			$custom_query = new \WP_Query( $args );
			if (! is_wp_error( $custom_query ) && $custom_query->have_posts()) {
				wp_cache_set( $k, $custom_query, 'irecco_theme' );
			}
		}
		...
		if ($query->have_posts()) :
		  echo '<section class="wgl_cpt_section">';
			echo '<div class="blog-posts">';
				...
				if (! empty($blog_title) || ! empty($blog_subtitle)) {
				  echo '<div class="wgl_module_title item_title">';
					if (! empty($blog_title)) {
						echo '<h3 class="irecco_module_title blog_title">', wp_kses( $blog_title, $allowed_html ), '</h3>';
					}
				...
				echo '<div class="container-grid row ', esc_attr($row_class) ,'">',
					\iRecco_Theme_Helper::render_html($blog_items),
				'</div>';

			echo '</div>';

			if ($blog_navigation == 'pagination') {
				echo \iRecco_Theme_Helper::pagination('10', $query, $blog_navigation_align);
			}
			...
			if ($blog_navigation == 'load_more') {
				...
				echo \iRecco_Theme_Helper::load_more($wgl_blog_atts, $name_load_more, $class);
			}
