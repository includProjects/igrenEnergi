...
if (! class_exists('iRecco_Single_Post')) {
	/**
	* @class        iRecco_SinglePost
	* @version      1.0
	* @category     Class
	* @author       WebGeniusLab
	*/
	...
	class iRecco_SinglePost
	{
		...
		public function render_featured(
			...
			if (!empty($media) && !$oembed_error) {
				echo '<div class="blog-post_media">',
					$bg_read_more ? '<a href="'.esc_url(get_permalink()).'" class="bg-read-more">'.esc_html__('READ MORE', 'irecco').'</a>' : '',
		...
		public function render_bg(
			...
			$featured_image = iRecco_Theme_Helper::options_compare('featured_image_type', 'mb_featured_image_conditional', 'custom');
			if ($featured_image == 'replace') {
				$featured_image_replace = iRecco_Theme_Helper::options_compare('featured_image_replace', 'mb_featured_image_conditional', 'custom');
			}
			...
			if ($media && $show_media) {
				if ($this->post_format === 'video' && $video_style == 'bg_video') {
					echo iRecco_Theme_Helper::render_html($media);
					...
					switch($this->post_format) {
						...
						case 'video':
							echo '<div class="blog-post-featured_media">',
								iRecco_Theme_Helper::render_html($media),
							'</div>';
							break;
					}
				}
			...
			if (is_single() && $video_style != 'bg_video') {
				...
				if ($video_style == 'bg_video') {
					...
					if (has_post_thumbnail()) {
						...
						$output .= '<div class="wgl-video_popup stick-bot-left with_image">';
						  $output .= '<div class="videobox_content">';
		...
		public function featured_quote()
		{
			...
			if (strlen($quote_author)) : ?>
				<div class="blog-post_quote-author"><?php
					echo iRecco_Theme_Helper::render_html($autor_avatar);
		...
		public function image_size_render($image_size)
		{
			...
			switch ($image_size) {
				...
				case 'irecco-840-620': $arr = [ 'width' => '840', 'height' => '620' ]; break;
				case 'irecco-740-560': $arr = [ 'width' => '740', 'height' => '560' ]; break;
				case 'irecco-700-550': $arr = [ 'width' => '700', 'height' => '550' ]; break;
				case 'irecco-440-440': $arr = [ 'width' => '440', 'height' => '440' ]; break;
				case 'irecco-420-300': $arr = [ 'width' => '420', 'height' => '300' ]; break;
				case 'irecco-180-180': $arr = [ 'width' => '180', 'height' => '180' ]; break;
			} 
			return $arr; 
		}
		...
		public function image_size_render_bg($image_size)
		{
			...
			switch ($image_size) {
				...
				case 'irecco-740-560': $arr = [ 'width' => '740', 'height' => '560' ]; break;
				case 'irecco-700-550': $arr = [ 'width' => '700', 'height' => '550' ]; break;
				case 'irecco-740-830': $arr = [ 'width' => '740', 'height' => '830' ]; break;
			}
			return $arr; 
		}
		...
		public function render_post_meta($args = false)
		{
			...
			if ($this->show_meta_date || $this->show_meta_author || $this->show_meta_comments || $this->show_meta_likes) :
			  echo '<div class="meta-wrapper post-meta-data">';
				...
				if ($this->show_meta_comments) {
					...
					echo '<span class="comments_post">',
						'<a href="', esc_url(get_comments_link()), '" title="', esc_attr__( 'Leave a reply', 'irecco'), '">',
							esc_html($comments_num),
							' ',
							esc_html( _n('Comment', 'Comments', $comments_num, 'irecco')  ),
						'</a>',
					'</span>';
				}
		...
		public function render_post_cats()
		{
			...
			if ($categories = get_the_category()) {
				foreach ($categories as $category) :
					...
					echo '<span>',
						'<a', (! empty($color) ? ' style="color: #'.esc_attr($color).';"' : "" ),' href="', esc_url(get_category_link($category->term_id)), '">',
							$category->cat_name,
						'</a>',
					'</span>';
				endforeach;
		...
		public function render_excerpt(
			...
			if ((bool)iRecco_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = iRecco_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
		...
		public function get_post_views($postID)
		{
			...
			$title = __( 'Total Views', 'irecco');
			$text = ' <span class="counts_text">' .esc_html( _n( 'View', 'Views', (int)$count, 'irecco') ). '</span>';
		...
		public function render_author_info()
		{
			...
			$name_html = ! empty($user_first) || ! empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">' . esc_html__('About', 'irecco') . '</span>'.$user_first.' '.$user_last.'</h5>' : '';
