...
if (! class_exists('iRecco_Theme_Helper')) {
	class iRecco_Theme_Helper
	{
        ...
        public static function get_option($name) { 
            if (  class_exists( 'Redux' ) && class_exists( 'iRecco_Core_Public' ) ) {

                // Customizer
                if (!empty($GLOBALS['irecco_set']) && $GLOBALS['irecco_set'] != NULL) {
                    $theme_options = $GLOBALS['irecco_set'];
                } else {
                    $theme_options = get_option( 'irecco_set' );
                }

                if (empty($theme_options)) {
                    $theme_options = get_option( 'irecco_default_options' );
                }

                return isset($theme_options[$name]) ? $theme_options[$name] : null;  

            }else{
                $default_option = get_option( 'irecco_default_options' );
		...
		{
			$image = iRecco_Theme_Helper::get_option($name."_bg_image");
		...
		public static function pagination($range = 5, $query = false, $alignment = 'left')
		{
			...
			$test_pag = paginate_links(array(
				...
				'current'   => max( 1, $paged ),
				'total'     => $max_page,
				'prev_text' => '<i class="fa flaticon-down"></i>',
				'next_text' => '<i class="fa flaticon-down"></i>',
			));
			$test_comp = '';
		...
		{
			$name_load_more = ! empty($name_load_more) ? $name_load_more : esc_html__( 'Load More', 'irecco' );
		...
		public static function render_sidebars($args = 'page')
		{
			...
			$irecco_core = class_exists('iRecco_Core');  
			...
			if (function_exists('is_shop') && is_shop() && ! $irecco_core) {
		...
		public static function posted_meta_on()
		{
			...
			printf('<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__( 'Published in', 'irecco' ) . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
			...
			if ($metadata) {
				printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s" title="%2$s">%1$s %3$s &times; %4$s</a></span>',
					esc_html_x( 'Full size', 'Used before full size attachment link.', 'irecco' ),
			...
			edit_post_link(
				sprintf(
					...
					wp_kses( __( 'Edit<span class="screen-reader-text"> "%s"</span>', 'irecco' ), $allowed_html ) ,
						get_the_title()
		...
		public static function enqueue_css($style)
		{
			if (! empty($style)) {
				...
				$css = ob_get_clean();
				$css = apply_filters( 'irecco_enqueue_shortcode_css', $css, $style );
			}
		}
