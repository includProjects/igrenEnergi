...
*
*
* @class        iRecco_Woocoommerce
* @version      1.0
* @category     Class
* @author       WebGeniusLab
*/

if ( ! class_exists('iRecco_Woocoommerce') ) {
	class iRecco_Woocoommerce{
	    /**
		...
		*/
		...
		public function setup() {
			...
			add_theme_support( 'woocommerce', apply_filters( 'irecco_woocommerce_args', array(
				'single_image_width'    => 1080,
				'thumbnail_image_width' => 540,
				'gallery_thumbnail_image_width' => 240,
				'product_grid'          => array(
					'default_columns' => (int) iRecco_Theme_Helper::get_option('shop_column'),
					'default_rows'    => 4,
					'min_columns'     => 1,
					'max_columns'     => 6,
					'min_rows'        => 1,),
		...
		function review_comments_meta_info($comment){
			...
			if ( '0' === $comment->comment_approved ) { ?>
				<em class="woocommerce-review__awaiting-approval">
					<?php esc_html_e( 'Your review is awaiting approval', 'irecco' ); ?>
				</em>
			...
			<?php } else { ?>
				...
				if ( 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified ) {
					echo '<em class="woocommerce-review__verified verified">(' . esc_attr__( 'verified owner', 'irecco' ) . ')</em> ';
				}
				?>
		...
		function wgl_filter_comments($comment_form){
			...
			$comment_form = array(
				'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'irecco' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'irecco' ), get_the_title() ),
				'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'irecco' ),
				'title_reply_before'   => '<span id="reply-title" class="comment-reply-title">',
				'title_reply_after'    => '</span>',
				'comment_notes_after'  => '',
				'fields'               => array(
					'author' => '<p class="comment-form-author">' . '<label for="author"></label> ' .
					'<input id="author" name="author" placeholder="'.esc_attr__( 'Name', 'irecco' ).'" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" aria-required="true" required /></p>',
					'email'  => '<p class="comment-form-email"><label for="email"></label> ' .
					'<input id="email" name="email" placeholder="'. esc_attr__( 'Email', 'irecco' ).'" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" aria-required="true" required /></p>',
				),
				'label_submit'  => esc_html__( 'Submit', 'irecco' ),
				'logged_in_as'  => '',
				'comment_field' => '',
			);
			...
			if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
				...
				$comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'irecco' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
			}

			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
				$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'irecco' ) . '</label><select name="rating" id="rating" aria-required="true" required>
				<option value="">' . esc_html__( 'Rate&hellip;', 'irecco' ) . '</option>
				<option value="5">' . esc_html__( 'Perfect', 'irecco' ) . '</option>
				<option value="4">' . esc_html__( 'Good', 'irecco' ) . '</option>
				<option value="3">' . esc_html__( 'Average', 'irecco' ) . '</option>
				<option value="2">' . esc_html__( 'Not that bad', 'irecco' ) . '</option>
				<option value="1">' . esc_html__( 'Very poor', 'irecco' ) . '</option>
				</select></div>';
			}

			$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" placeholder="'.esc_attr__( 'Your review', 'irecco' ).'" required></textarea></p>';
			return $comment_form;
		}
		...
		function wrapper_catalog_shop( $ob_get_clean ){
			$animation = (bool) iRecco_Theme_Helper::get_option('use_animation_shop');
			$animation_style = iRecco_Theme_Helper::get_option('shop_catalog_animation_style');
		...
		public function loop_products_per_page() {
			return (int) iRecco_Theme_Helper::get_option('shop_products_per_page');
		}
		/**/
		/* \LOOP */
		/**/
		...
		public function related_products_args( $args ) {
			$args = array(
				'posts_per_page' => (int) iRecco_Theme_Helper::get_option('shop_r_products_per_page'),
				'columns'        => (int) iRecco_Theme_Helper::get_option('shop_related_columns'),
			);
		...
		public function loop_columns( $args ) {
			$columns = (int) iRecco_Theme_Helper::get_option('shop_column'); // 3 products per row
			return $columns;
		}

		public function wgl_product_columns_wrapper() {
			$columns = (int) iRecco_Theme_Helper::get_option('shop_column');
		...
		public function init_template(){
			...
			$sb = iRecco_Theme_Helper::render_sidebars('shop_'.$shop_template);
		...
		public function wgl_filter_pagination()
		{
			...
			return array( // WPCS: XSS ok.
				...
				'total'     => $total,
                'prev_text' => '<i class="fa flaticon-left-arrow"></i>',
                'next_text' => '<i class="fa flaticon-right-arrow"></i>',
				'type'      => 'list',
				'end_size'  => 3,
				'mid_size'  => 3,
			);
...
new iRecco_Woocoommerce ( );

// iRecco Woocoommerce Helpers

if ( ! function_exists( 'irecco_woocommerce_breadcrumb' ) ) {
	/**
	 * Output the WooCommerce Breadcrumb.
	 *
	 * @param array $args Arguments.
	 */
	function irecco_woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => esc_html_x( 'Home', 'breadcrumb', 'irecco' ),
		) ) );
		...
		if ( ! empty( $breadcrumb ) ) {
			...
			$out .= iRecco_Theme_Helper::render_html($wrap_before);

			foreach ( $breadcrumb as $key => $crumb ) {

				$out .=  iRecco_Theme_Helper::render_html($before);
				...
				$out .=  iRecco_Theme_Helper::render_html($after);

				if ( sizeof( $breadcrumb ) !== $key + 1 ) {
					$out .=  iRecco_Theme_Helper::render_html($delimiter);
				}
			}
			$out .=  iRecco_Theme_Helper::render_html($wrap_after);
		}
